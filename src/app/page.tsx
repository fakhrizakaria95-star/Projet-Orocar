...
export default function Page() {
  return (
    <main className="min-h-screen bg-[#050505] text-white font-sans flex flex-col">
      {/* Barre de Navigation */}
      <header className="flex justify-between items-center px-12 py-8 w-full relative z-50">
        <div className="text-2xl font-black tracking-widest italic">OROCAR</div>
        <nav className="flex items-center gap-6">
          <Link href="/ajouter" 
            className="text-[10px] font-bold px-5 py-2.5 rounded-full border border-orange-500/30 hover:bg-gradient-to-r from-orange-500 to-rose-500 transition-all duration-300 uppercase tracking-widest cursor-pointer">
          ...
          <Link href="/auth" 
            className="text-[10px] font-bold bg-white text-black px-6 py-2.5 rounded-full hover:bg-gradient-to-r from-orange-400 to-rose-400 hover:text-white transition-all duration-300 uppercase cursor-pointer">
            Connexion
          </Link>
        </nav>
      </header>
      ...
      <footer className="py-10 flex flex-col items-center w-full">
        <p className="text-zinc-700 text-[9px] tracking-[0.3em]">© 2026 OROCAR. PREMIUM MOBILITY</p>
      </footer>
    </main>
  );
}